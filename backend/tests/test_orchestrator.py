...
DB, with only the actual Gemini network calls mocked out (extraction's
combined call, confirmation's classification call). Everything else -- DB
writes, consistency detection, the deterministic clarifying-question path,
the completeness-triggers-readback path, and the confirm/decline flow --
runs for real.
"""
...
from app.models.schema import ExtractedTurn, FaceParameters, Spacing
...
def _mock_extraction(monkeypatch, updates: FaceParameters, reply_text: str = "ok, go on") -> None:
    monkeypatch.setattr(
        "app.agents.extraction.generate_structured",
        AsyncMock(return_value=ExtractedTurn(updates=updates, reply_text=reply_text)),
    )
...
async def test_normal_turn_persists_feature_version_and_uses_model_reply(db_session, monkeypatch):
    case, session = _seed_case_and_session(db_session)

    _mock_extraction(
        monkeypatch,
        FaceParameters(face_shape="oval", face_shape_verbatim="kind of oval"),
        reply_text="Got it. What about the eyes?",
    )
...
async def test_contradiction_returns_deterministic_question_overriding_model_reply(db_session, monkeypatch):
    case, session = _seed_case_and_session(db_session)
    db_session.add(
        FeatureVersion(
    ...
    # Model's own reply_text is deliberately something that would fail the
    # assertions below if it were used -- proves the deterministic override
    # wins, not just that it happens to match.
    _mock_extraction(
        monkeypatch,
        FaceParameters(eyes_spacing=Spacing.wide_set, eyes_spacing_verbatim="wide apart eyes"),
        reply_text="Sure, noted!",
    )
    ...
    assert result.reply_text != "Sure, noted!"
...
async def test_complete_description_triggers_readback_overriding_model_reply(db_session, monkeypatch):
    case, session = _seed_case_and_session(db_session)

    _mock_extraction(monkeypatch, COMPLETE_ENOUGH_PARAMS, reply_text="Sure, noted!")
...
async def test_confirmation_declined_falls_through_to_correction(db_session, monkeypatch):
    ...
    _mock_extraction(monkeypatch, FaceParameters(facial_hair="mustache", facial_hair_verbatim="had a mustache"))

    result = await orchestrator.handle_turn(db_session, session, "oh also, he had a mustache")

    db_session.refresh(session)
    # description is still complete after the addition -> re-triggers readback, not the model's own reply
    assert session.awaiting_confirmation is True
    assert session.status != SessionStatus.confirmed_by_witness
...
async def test_confirmation_declined_with_genuine_contradiction_still_asks_deterministically(db_session, monkeypatch):
    ...
    _mock_extraction(monkeypatch, FaceParameters(nose_size="large", nose_size_verbatim="actually big"))
